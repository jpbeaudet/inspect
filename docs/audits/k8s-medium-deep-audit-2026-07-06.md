...
| R4 | R | Med | Dual-verb k8s branch-detection copy-pasted ~15× (near byte-identical, double-resolves); 7 k8s-only verbs use the opposite swallow-vs-propagate convention → one `k8s_route()` helper | ✅ **FIXED** — `dispatch::require_k8s()` collapses the 6 byte-identical k8s-only preambles (describe/events/top/scale/rollout/delete); dual-runtime verbs keep their `if let Ok` fall-through as a distinct outcome (Rule 13). R6 additionally collapsed the write failure-branch |
